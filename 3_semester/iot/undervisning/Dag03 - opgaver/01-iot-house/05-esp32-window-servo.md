# 🪟 Opgave 05 – Styr Vindue-Servo med ESP32

I denne opgave skal du programmere ESP32 til at **modtage** MQTT-kommandoer og styre en servo motor der åbner og lukker et vindue. Servoen forbindes til GPIO 5.
...
```python
# ESP32 MQTT Subscriber - Vindue Servo Kontrol
...
MQTT_TOPIC_WINDOW = b"stud/esp32/dit_navn/window"
CLIENT_ID = b"esp32_window_dit_navn"
...
def main():
    ...
    print(f'✅ MQTT subscribed til: {MQTT_TOPIC_WINDOW}')
    
    # Initial position: lukket
    set_servo_angle(WINDOW_CLOSED)
...
1. **Rediger følgende i koden:**
   ...
   - `dit_navn` i topic → Dit eget navn (fx `stud/esp32/anders/window`)
   - `CLIENT_ID` → Unikt ID (fx `esp32_window_anders`)

2. **Gem filen:**
   - **File → Save as → MicroPython device**
   - Gem som `window_servo.py` eller `main.py`
...
✅ MQTT subscribed til: b'stud/esp32/anders/window'
🪟 Vindue servo: 0°
...
✅ Din ESP32 styrer nu vindue-servoen via MQTT!
...
2. Publish til topic `stud/esp32/dit_navn/window`
3. Send kommandoer:
   - `OPEN` = Åbn vindue (90°)
   - `CLOSE` = Luk vindue (0°)
   - `45` = Halvt åbent (45°)
...
**Opret flow:**
```
[Inject "OPEN"] → [MQTT Out: topic "window"]
[Inject "CLOSE"] → [MQTT Out: topic "window"]
[Slider 0-180] → [MQTT Out: topic "window"]
```

**Trin-for-trin:**
1. Opret `inject` node med `msg.payload = "OPEN"`
2. Opret `inject` node med `msg.payload = "CLOSE"`
3. Opret `inject` node med `msg.payload = "45"` (halvt åbent)
4. Opret `mqtt out` node:
   - Broker: `test.mosquitto.org`
   - Topic: `stud/esp32/dit_navn/window`
5. Forbind alle inject-noder til mqtt out
6. Deploy og test!

**Bonus - Slider:**
Du kan bruge en dashboard slider node (0-180) til at styre vinklen præcist!
...
2. **Pulse width bestemmer vinkel:**
   - 0.5ms pulse = 0° (helt lukket)
   - 1.5ms pulse = 90° (midterposition)
...
4. **Kommando-typer:**
   - **Preset**: `OPEN` / `CLOSE` - nem styring
   - **Præcis**: `0` til `180` - fuld kontrol over vinkel

**MQTT Kommandoer:**
- `OPEN` → 90° (vindue åbent)
- `CLOSE` → 0° (vindue lukket)  
- `0-180` → Præcis vinkel (fx `45` for halvt åbent)

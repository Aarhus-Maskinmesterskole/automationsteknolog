# Opgave 9: ESP32 + LCD som Modbus TCP server

Mål: skift tekst på LCD fra Node-RED.

## Før du starter

Kopiér også `lcd_api.py` og `i2c_lcd.py` til ESP32. De ligger i `python`-mappen.

## Adresser

* Coil `0` = `true` viser `HELLO`, `false` viser `STOP`
...
`true` viser `HELLO`. `false` viser `STOP`.